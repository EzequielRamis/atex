...
The VTEX Storefront Framework is a piece of ...software, that lets you develop a store website using React under the hood. What do I mean by "under the hood"? Well...
...
You might think that there would be some way to look at the code generated by the builder, but all of that is handled by the VTEX servers, alongside hydration and HMR (which really should be called Cold Module Replacement, since updates can take several seconds). It is possible to write external JavaScript code if VTEX doesn't provide a given component, at the end of the day in this JSON DSL you're just calling components.
...
Enough ranting, let's see a posible solution to this. I had to put myself in Tom's shoes, so don't expect it to be a perfect one.
...
To render more than one block without a parent, you can use standard Fragment syntax.
...
As explained in [Usage/JSON](#json), ATEX supports `.json` files which behave the same as if they were at `store`. So, if you're considering a migration, you can copy all files from `store` to `atex/src` and slowly replace them with ATEX modules.
